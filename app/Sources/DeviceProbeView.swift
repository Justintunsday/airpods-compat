    ...
    var body: some View {
        List {
            Section(footer: Text("探测只在本机进行，不上传任何数据。BLE 扫描需要蓝牙权限；已连接设备需要私有 API 可用（越狱环境成功率更高）。")) {
                ...
                Button("刷新已连接设备") { probe.refreshConnectedDevices() }
            ...
            Section(header: Text("已连接 / 已配对设备"),
                    footer: Text(probe.bluetoothProbeStatus ?? "")) {
                if probe.connectedDevices.isEmpty {
                    Text("未枚举到设备").foregroundColor(.secondary)
                }
                ForEach(probe.connectedDevices) { device in
                    VStack(alignment: .leading, spacing: 3) {
                        HStack {
                            ...
                            Spacer()
                            Text(device.connected ? "已连接" : "已配对")
                                .font(.caption2)
                                .foregroundColor(device.connected ? .green : .secondary)
                        }
                        Text(device.describe(catalog: .shared))
                            .font(.caption)
            ...
            Section(header: Text("附近 BLE 设备"),
                    footer: Text("AirPods 在未连接/开盖时会广播近场配对包（mfr 以 4c00 开头），已连接时通常不再广播，属正常现象。")) {
                ...
                ForEach(probe.nearby) { device in
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        if !device.rawManufacturerData.isEmpty {
                            Text("mfr=\(device.rawManufacturerData)")
                                .font(.system(.caption2, design: .monospaced))
                                .foregroundColor(.secondary)
                                .textSelection(.enabled)
                        }
                        if !device.advertisementDump.isEmpty {
                            DisclosureGroup("原始广播") {
                                Text(device.advertisementDump)
                                    .font(.system(.caption2, design: .monospaced))
                                    .textSelection(.enabled)
                            }
                            .font(.caption2)
